        ...
        if ($insert_comment) {
            ...
            if(!isset($_POST['type'])){
                $comment     = PT_LoadPage('watch/comments', array(
                    'ID' => number_format($get_comment->id),
                ...
                $comment     = PT_LoadPage('watch/comments-mob', array(
                    'ID' => number_format($get_comment->id),
                    'TEXT' => PT_Markup($get_comment->text),
                    'TIME' => PT_Time_Elapsed_String($get_comment->time),
                    'USER_DATA' => $user_data,
...
if (!empty($_POST['gif_id']) && !empty($_POST['text'])) {
    $text = PT_Secure($_POST['text']);
    $id   = PT_Secure($_POST['gif_id']);
    
    //$video_data = $pt->get_video = $db->where('id', $id)->getOne(T_VIDEOS);
    $video_data = $pt->get_gif = PT_GetGifByID($id, 0, 0, 2);
    ...
    if (!empty($video_data) ) {
        ...
        if ($insert_comment) {
            ...
            $comment     = PT_LoadPage('gif/comments', array(
                'ID' => $get_comment->id,
                'TEXT' => PT_Markup($get_comment->text),
                'TIME' => PT_Time_Elapsed_String($get_comment->time),
                'USER_DATA' => $user_data,
            ...
            // if ($video_data->user_id != $user->id) {
            //     RegisterPoint($video_data->id, "comments");
            //     $type    = 'commented_ur_gif';
            //     $uniq_id = $video_data->video_id;
            //     $notif_data = array(
            //         'notifier_id' => $pt->user->id,
            //         'recipient_id' => $video_data->user_id,
            //         'type' => $type,
            //         'url' => "gif/$uniq_id&cl=$insert_comment",
            //         'gif_id' => $id,
            //         'time' => time()
            //     );
