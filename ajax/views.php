...
if (!empty($_GET['type_']) && $_GET['type_'] == 'add' && !empty($_SESSION['finger'])) {
	...
	}elseif(isset($_POST['gif_id'])){
		$video = PT_GetGifByID($_POST['gif_id'], 0, 0, 2);
		if (!empty($video)) {
			$finger = $_SESSION['finger'];
			$add = true;
			if (IS_LOGGED == true) {
				$is_viewed = $db->where('user_id',$pt->user->id)->where('gif_id',$video->id)->where('time',time() - 31556926,'>=')->getValue(T_VIEWS,"count(*)");
				if ($is_viewed > 0) {
					$add = false;
				}
			}else{
				$is_viewed = $db->where('fingerprint',$finger)->where('gif_id',$video->id)->where('time',time() - 31556926,'>=')->getValue(T_VIEWS,"count(*)");
				if ($is_viewed > 0) {
					$add = false;
				}else{
					// if (!empty($_COOKIE['views']) && !empty($_SESSION['views'])) {
			...
			if ($add == true) {
				...
				setcookie("views", $data_s, time() + 31556926, '/');
				$_SESSION['views'] = $data_s;
				$data_info = array('gif_id' => $video->id,
								   'fingerprint' => $finger,
								   'time'    => time());
				if (IS_LOGGED == true) {
					$data_info['user_id'] = $pt->user->id;
				...
				$db->insert(T_VIEWS,$data_info);
				$db->where('id', $video->id)->update(T_GIFS, array('views' => $db->inc(1)));
				$new_video = $db->where('id', $video->id)->getOne(T_GIFS);
				RegisterPoint($video->id, "gif");
				$data['count'] = $new_video->views;
				$data['status'] = 200;
			}
		}
