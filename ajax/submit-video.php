...
if (empty($error)) {
    ...
    foreach($pt->categories as $key => $category){
        if(isset($_POST['category_id_'.$key])){
            if(empty($category_id)){
                $category_id = $_POST['category_id_'.$key];
            }else{
                $category_id =  $category_id .','. $_POST['category_id_'.$key];
            }
        }
    }
    ...
    if ($insert) {
        ...
        if (!empty($_POST['uploaded_id']) && is_numeric($_POST['uploaded_id']) && $_POST['uploaded_id'] > 0) {
            $db->where('id',PT_Secure($_POST['uploaded_id']))->where('user_id',(isset($pt->user->id) ? $pt->user->id : 0))->where('path',PT_Secure($_POST['video-location']))->delete(T_UPLOADED);
        }
