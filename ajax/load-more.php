...
elseif (!is_numeric($_POST['last_id']) || $_POST['last_id'] < 0) {
   ...
   //              }
		...
		if ($request === true) {
			...
			if (empty($_POST['cat'])){
				$articles    = $db->where('active', '1')->where('id', $id, '<')->where('user_id',$pt->blocked_array , 'NOT IN')->orderby('id', 'DESC')->get(T_GIFS, 10);
			}

			else if (!empty($_POST['cat']) && is_numeric($_POST['cat'])) {	
				$category_id = PT_Secure($_POST['cat']);
				$articles    = $db->where('active', '1')->where('id', $id, '<')->where('category', $category_id)->where('user_id',$pt->blocked_array , 'NOT IN')->orderby('id', 'DESC')->get(T_GIFS, 10);
			}
			...
			if (count($articles) > 0) {
			    ...
			    $data = array('status' => 200, 'posts' => $html_posts);
			}
