...
$config['theme_url']      = $site_url . '/pages/' . $config['theme'];
    ...
    foreach ($all_categories as $key => $value) {
        ...
        if ($value->lang_key != 'other') {
            if (!empty($value->lang_key) && !empty($lang->{$value->lang_key})) {
                ...
                $category_images[$value->lang_key] = $value->image;
            }
...
$pt->categories  = ToObject($categories);
$pt->category_images  = ToObject($category_images);
...
$pt->theme_using = 'design';
$path_to_details = './pages/' . $config['theme'] . '/fonts/info.json';
