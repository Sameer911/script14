...
function PT_UserData($user_id = 0, $options = array()) {
    ...
    $fetched_data->url    = PT_Link('user/'.$fetched_data->username);
...
function PT_GetVideoByID($video_id = '', $add_views = 0, $likes_dislikes = 0, $run_query = 1, $short_id = 0) {
    ...
    if(isset($get_video->is_guest) && $get_video->is_guest == 0 ){
        if ($short_id == 1) {
    ...
    if (!empty($get_video)) {
        ...
        if($get_video->user_id != 0){
            $get_video->owner              = PT_UserData($get_video->user_id);
        }else{
            $get_video->owner = new StdClass();
            $get_video->owner->name        = 'Guest';
        }
        ...
        $get_video->meta_time    = date('Y-m-d\TH:i:sO', $get_video->time);
        $get_video->time_alpha    = gmdate('d M Y', $get_video->time);
        $get_video->time_ago      = PT_Time_Elapsed_String($get_video->time);
        $get_video->categories = array();
        $get_video->category_name = '';
        if( strpos($get_video->category_id, ',') == 1){
            $get_video->categories = (!empty($get_video->category_id)) ? explode(',', $get_video->category_id) : array(); 
        }else{
...
function PT_GetGifByID($gif_id = '', $add_views = 0, $likes_dislikes = 0, $run_query = 1, $short_id = 0) {
    global $pt, $db, $categories;
    ...
    // }else if($short_id == 0){
    //     $get_gif = $db->where('id', $gif_id)->getOne(T_GIFS);
    // } else {
    //     $get_gif = $gif_id;
    // }
    if ($short_id==1) {
        $get_gif = $db->where('short_id', $gif_id)->getOne(T_GIFS);
    }else if ($run_query == 1) {
        $get_gif = $db->where('gif_id', $gif_id)->getOne(T_GIFS);
    }else if ($run_query == 2) {
        $get_gif = $db->where('id', $gif_id)->getOne(T_GIFS);
    }else{
        $get_gif = $gif_id;
    ...
    if (!empty($get_gif)) {
        ...
        $get_gif->gif_location = PT_GetMedia($get_gif->gif_location);
       
        $get_gif->url                = PT_Link('gif/' . $get_gif->gif_id);
        if ($pt->config->seo_link == 'on') {
            $get_gif->url = PT_Link('gif/' . PT_Slug($get_gif->title, $get_gif->gif_id));
        }
            
        $get_gif->owner              = PT_UserData($get_gif->user_id);
        $get_gif->is_liked           = 0;
        $get_gif->is_disliked        = 0;
        $get_gif->is_owner           = false;
        $get_gif->is_purchased = 0;
        
        if (IS_LOGGED == true) {
            $get_gif->is_liked    = $db->where('user_id', $pt->user->id)->where('gif_id', $get_gif->id)->where('type', 1)->getValue(T_DIS_LIKES, 'count(*)');
            $get_gif->is_disliked = $db->where('user_id', $pt->user->id)->where('gif_id', $get_gif->id)->where('type', 2)->getValue(T_DIS_LIKES, 'count(*)');
            if ($get_gif->owner->id == $pt->user->id || PT_IsAdmin()) {
                $get_gif->is_owner           = true;
            }
        }
        $get_gif->time_alpha    = gmdate('d M Y', $get_gif->time);
        $get_gif->time_ago      = PT_Time_Elapsed_String($get_gif->time);
        $get_gif->category_name = (!empty($categories[$get_gif->category])) ? $categories[$get_gif->category] : ''; 
        if ($likes_dislikes == 1) {
            $db->where('gif_id', $get_gif->id);
            $db->where('type', 1);
            $get_gif->likes = $db->getValue(T_DIS_LIKES, 'count(*)');
            
            $db->where('gif_id', $get_gif->id);
            $db->where('type', 2);
            $get_gif->dislikes = $db->getValue(T_DIS_LIKES, 'count(*)');
            
            $total                    = $get_gif->likes + $get_gif->dislikes;
            $get_gif->likes_percent = 0;
            if ($get_gif->likes > 0) {
                $get_gif->likes_percent = round(($get_gif->likes / $total) * 100);
            }
            $get_gif->dislikes_percent = 0;
            if ($get_gif->dislikes > 0) {
                $get_gif->dislikes_percent = round(($get_gif->dislikes / $total) * 100);
            }
            
            if ($get_gif->likes_percent == 0 && $get_gif->dislikes_percent == 0) {
                $get_gif->dislikes_percent = 100;
                $get_gif->likes_percent    = 0;
            }
        }
        return $get_gif;
...
function PT_ShareFileFromUrl($url){
    global $pt, $mysqli;
    $allowed = '';
    ...
    $folder   = 'videos';
    $fileType = 'video';
    $new_string        = pathinfo(basename($url), PATHINFO_FILENAME) . '.' . strtolower(pathinfo(basename($url), PATHINFO_EXTENSION));
    $file_extension    = pathinfo($new_string, PATHINFO_EXTENSION);
    $allowed           = 'mp4';
    ...
    if ($file) { 
      ...
      if ($newf) 
      while(!feof($file)) { 
       $result = fwrite($newf, fread($file, 1024 * 8 ), 1024 * 8 ); 
      } 
    } 
    if ($file) { 
      fclose($file); 
...
function PT_ShareFileInChunks($data = array(),$REQUEST,$type = 0){
    ...
    $data['name'] = isset($_REQUEST["name"]) ? $_REQUEST["name"] : $data["name"];
    $new_string        = pathinfo($data['name'], PATHINFO_FILENAME) . '.' . strtolower(pathinfo($data['name'], PATHINFO_EXTENSION));
    $extension_allowed = explode(',', $allowed);
    $file_extension    = pathinfo($new_string, PATHINFO_EXTENSION);
    if (!in_array($file_extension, $extension_allowed)) {
    ...
    $out = @fopen("{$filename}.part", ($chunk == 0 ? "wb" : "ab"));
        if ($out) {$in = @fopen($data['file'], 'rb');if ($in) {while ($buff = fread($in, 4096)){@fwrite($out, $buff);}} else{return array( 'error' => 'Failed to open input stream.');}@fclose($in);@fclose($out);@unlink($_FILES['file']['tmp_name']);} else{return array( 'error' => 'Failed to open output stream.');}if (!$chunks || $chunk == $chunks - 1) {@rename("{$filename}.part", $filename);$last_data             = array();$last_data['filename'] = $filename;$last_data['name']     = $data['name'];return $last_data;}else{return array( 'error' => 'Upload Failed');}
}
