...
if ($page == 'trending') {
    ...
    if (!empty($_GET['id'])) {
        if (in_array($_GET['id'], array_keys($categories))) {
            ...
            $category_image = $pt->category_images->{$cateogry};
            ...
            // pagination system 
                $videos = $db->where('privacy', 0)->where('user_id',$pt->blocked_array , 'NOT IN')->where('category_id like "'.'%'. $cateogry.'%' .'"')->where('is_movie',0)->where('live_time',0)->where('approved',1)->orderBy('id', 'DESC')->objectbuilder()->paginate(T_VIDEOS, $pt->page_number);
