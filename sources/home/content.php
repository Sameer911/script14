...
if (empty($guest_list)) {
    $db->where('privacy', 0);
    $guest_list = $db->where('is_movie', 0)->where('is_guest',1)->orderBy('views', 'DESC')->get(T_VIDEOS, $limit);
}

foreach ($guest_list as $key => $video) {
    $video = $pt->video =  PT_GetVideoByID($video, 0, 0, 0);
    $guest_list .= PT_LoadPage('home/list', array(
        'ID' => $video->id,
        'TITLE' => $video->title,
        'VIEWS' => number_format($video->views),
        'VIEWS_NUM' => number_format($video->views),
...
foreach ($categories as $cat_key => $cat_name) {
    if (IS_LOGGED == true && !empty($pt->user->fav_category)) {
        if (in_array($cat_key, $pt->user->fav_category)) {
            $db->where("category_id like' .'%'.$cat_key .'%'.'");
            $db->where('privacy', 0);
            $db->orderBy('id', 'DESC');
