...
if (empty($get_video)) {
    if(headers_sent()){
        echo '  <script> location.replace("'.PT_Link('404').'"); </script> ';
    }else{
      header("Location: " . PT_Link('404'));
    }
...
$related_videos    = $db->rawQuery("SELECT * FROM " . T_VIDEOS . " WHERE MATCH (title) AGAINST ('$query_video_title') AND user_id NOT IN (".implode(',', $pt->blocked_array).")  $not_in_query AND is_movie = 0 AND privacy = 0 ORDER BY `id` DESC LIMIT 20");
...
if (empty($related_videos)) {
    ...
    $related_videos = $db->where('category_id', $get_video->category_id)->where('user_id',$pt->blocked_array , 'NOT IN')->where('is_movie',0)->get(T_VIDEOS, 20);
}
...
foreach ($related_videos as $key => $related_video) {
    ...
    if($related_video->owner->name == 'Guest'){
        $video_data['USER_DATA'] = false;
     }else{
        $video_data['USER_DATA'] = $related_video->owner;
     }
    $video_sidebar .= PT_LoadPage('watch/video-sidebar',$video_data);
...
if (!empty($get_video_comments)) {
    ...
    foreach ($get_video_comments as $key => $comment) {
        ...
        $comments     .= PT_LoadPage('watch/comments', array(
            ...
            'VID_ID' => number_format($get_video->id)
        )); 

        $comments_mob     .= PT_LoadPage('watch/comments-mob', array(
            'ID' => number_format($comment->id),
            'TEXT' => PT_Markup($comment->text),
            'TIME' => $current_time,
            'USER_DATA' => $comment_user_data,
...
if(!empty($get_video->categories)){
    $video_categories = array();
    foreach($get_video->categories as $value ){
        foreach($pt->categories as $key => $val){
            if($key == $value){
                $video_categories[$key] = $val;
            }
        }
    }

    foreach ($video_categories as $key => $value) {
        $categories_list .= PT_LoadPage('watch/category', array(
            'CATEGORY' => $value,
            'CATEGORY_ID' => $key ,
        ));
        // $is_found = $db->where('lang_key',$key)->getValue(T_LANGS,'COUNT(*)');
        // if ($is_found == 0) {
        //     $db->where('id',$get_video->id)->update(T_VIDEOS,array('category_id' => $get_video->categories.',other','sub_category' => ''));
        //     $get_video->category_name = "";
        //     $get_video->category_id = 'other';
        // }
    }
}else if(!empty($get_video->category_name)){
    $categories_list .= PT_LoadPage('watch/category', array(
...
$pt->content = PT_LoadPage("watch/$content_page", array(
    ...
    'VIDEO_COMMENTS' => PT_LoadPage('watch/video-comments',array(
        ...
        'URL' => $get_video->url,
        'VIDEO_ID' => number_format($get_video->id)
    )),
    'VIDEO_COMMENTS_MOB' => PT_LoadPage('watch/video-comments-mob',array(
        'COUNT_COMMENTS' => $pt->count_comments,
        'COMMENTS' => $comments_mob,
        'PINNED_COMMENTS' => $pinned_comments,
        'URL' => $get_video->url,
        'VIDEO_ID' => number_format($get_video->id)
    )),
    ...
    'AD_P_LINK' => "$ad_link",
    'AD_SKIP' => $ad_skip,
    'AD_SKIP_NUM' => number_format($ad_skip_num),
    'ADS' => $is_video_ad,
    ...
    'COUNTRIES' => $countries,
    'META_DURATION' => $get_video->duration,
    'META_TIME' => $get_video->meta_time,
