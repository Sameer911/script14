...
if($page== 'home'){
    if(isset($_GET['v']) || isset($_GET['g'])){
     }else{
        echo '<div id="skeleton">'. PT_LoadPage("skeleton/home",array()).'</div>';

     }
}
if($page== 'watch'){
    echo '<div id="skeleton">'. PT_LoadPage("skeleton/watch",array()).'</div>';
}
if($page== 'channel'){
    echo '<div id="skeleton">'. PT_LoadPage("skeleton/channel",array()).'</div>';
}
if($page== 'search'){
    echo '<div id="skeleton">'. PT_LoadPage("skeleton/search",array()).'</div>';
}else{
        echo '<div id="skeleton"> </div>';
}
...
if (!empty($_GET['g'])) {
    $video_short_id = PT_Secure($_GET['g']);
    $get_video = PT_GetGifByID($video_short_id, 0, 0, 1, 1);
...
$final_content = PT_LoadPage('container', array(
    ...
    'FOOTER_AD' => ($pt->page != 'register' && $pt->page != 'sign-in') ? PT_GetAd('footer') : '',
